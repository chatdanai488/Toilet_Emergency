...
class Notification:
    def __init__(self, root, master_app=None):
        try:
            ...
            self.pulse_colors = ["red", "blue", "green",
    ...
    def create_widgets(self):
        try:
            ...
            self.return_button = tk.CTkButton(
                self.sidebar, text="Return", fg_color="red", command=self.Return_To_Main)
            ...
            # ButtonFrame
            self.button_Frame = tk.CTkFrame(
                self.sidebar, width=270, height=270)
            ...
            self.frame_index = 0
            self.load_image = self.loading_canvas.create_image(
                200, 200, image=self.gif_frames[0])

            # Create Heading label
            self.Heading = tk.CTkLabel(
                self.Label_Frame, text="Notification", font=("Arial", 24, "bold"), width=250)
            self.Heading.grid(column=0, row=0)

            # Message Frame
            self.message_frame = tk.CTkFrame(
                self.sidebar, fg_color="lightblue")
            self.message_frame.grid(column=0, row=3, padx=5, sticky="nsew")

            # Right Side
            self.Notification_right_frame = tk.CTkFrame(
                self.root, fg_color="lightgreen")
    ...
    def animate(self):
        try:
            ...
            self.loading_canvas.itemconfig(
                self.load_image, image=self.gif_frames[self.frame_index])
    ...
    def create_button(self, Data, LocId):
        try:
            ...
            new_button.grid(column=self.column_count,
                            row=self.row_count, padx=2, pady=2)
            ...
            new_button.configure(command=lambda button=new_button,
                                 idx=self.count: self.show_canvas(button, idx))

            # Create associated canvas
            canvas = tk.CTkCanvas(self.Notification_right_frame,
            ...
            message = tk.CTkLabel(self.message_frame,
                                  text=Current_Message, width=270, height=40)
            message.grid(row=0, column=0, sticky="nsew")
            self.messages[self.count] = message
            message.grid_forget()

            delete_button = tk.CTkButton(
                self.Notification_right_frame, text="Delete", command=lambda idx=self.count: self.delete_item(idx))
    ...
    def reorganize_buttons(self):
        try:
            ...
            button_count = sum(1 for widget in self.button_Frame.winfo_children(
            ...
            if button_count == 0:
                ...
                # Add 4 to round up when there's a partial row
                self.row_count = (button_count + 4) // 5
                # Maximum of 5 buttons per row
                self.column_count = min(button_count, 5)
        except Exception as e:
    ...
    def add_canvas_component(self, data, canvas):
        try:
            ...
            image_id = current_canvas.create_image(
                0, 0, anchor="nw", image=self.canvas_images[image_name])
            self.canvas_image[self.count] = image_id
            current_canvas.configure(
                scrollregion=(0, 0, new_width, new_height))

            if current_data[9] == "square":
                x1, y1 = current_data[5] * \
                    new_width, current_data[6] * new_height
                x2, y2 = current_data[7] * \
                    new_width, current_data[8] * new_height
                shape_id = current_canvas.create_rectangle(
                    x1, y1, x2, y2, fill="red", stipple="gray50")
            elif current_data[9] == "line":
                x1, y1 = current_data[5] * \
                    new_width, current_data[6] * new_height
                x2, y2 = current_data[7] * \
                    new_width, current_data[8] * new_height
                shape_id = current_canvas.create_line(
                    x1, y1, x2, y2, fill="red")
            elif current_data[9] == "dot":
                x1, y1 = current_data[5] * \
                    new_width, current_data[6] * new_height
                shape_id = current_canvas.create_oval(
                    x1-2, y1-2, x1+2, y1+2, fill="red")
    ...
    def resize_canvas_component(self):
        try:
            if self.active_canvas:
                ...
                # Width is the difference between the x-coordinates
                ...
                self.active_canvas.itemconfig(
                    self.active_canvas_image, image=self.images)
                ...
                self.active_canvas.coords(
                    self.active_canvas_shape, new_shape_x1, new_shape_y1, new_shape_x2, new_shape_y2)
    ...
    def pulsate_shape(self):
        try:
            if self.active_canvas:
                ...
                self.active_canvas.itemconfig(
                    self.active_canvas_shape, fill=color)
